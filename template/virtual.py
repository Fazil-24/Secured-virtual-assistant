...
if __name__=='__main__':
    ...
    while True:
        ...
        if "good bye" in statement or "ok bye" in statement or "stop" in statement:
            ...
            print('your personal assistant  shutting down,Good bye')
            break
        ...
        if 'wikipedia' in statement:
            ...
            receiver=takeCommand()
            x = col.find_one({'email': receiver})
            print(x)
            speak("may i know your name")
            sender=takeCommand()
            #########
            #speak("Authenticating.......")
            y = col.find_one({'email': sender})
            z = col.find_one({'password': sender})
            print(y)
            s = smtplib.SMTP('smtp.gmail.com', 587) 
            s.starttls()
            s.login(sender,z)
            speak("what is your message that you would like to send")
            message = takeCommand()
            s.sendmail(y,x, message)
            s.quit()
